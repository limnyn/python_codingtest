...
# 현민이는 게임 캐릭터가 맴 안에서 움직이는 시스템을 개발 중이다. 
...
# 입력 조건
#     첫째 출에 맴의 세로 크기 N과 가로 크기 M을 공백으로 구분하여 입력한다. (N >= 3, M <= 50)
...
#             2 : 남쪽
#             3 : 서쪽
#     셋째 줄부터 맵이 육지인지 바다인지 입력한다. N개의 줄에 맵의 상태가 북쪽부터 남쪽 순서대로, 각 줄의 데이터는 서쪽부터 동쪽 순서대로 주어진다.
#         맵의 외곽은 항상 바다로 되어 있다.
#             0 : 육지
#             1 : 바다
#     처음에 게임 캐릭터가 위치판 칸의 상태는 항상 육지이다.
